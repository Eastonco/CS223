...
public:
	...
	void run_dijkstras(int starting_nodeId) {
		...
		DijkstraVertex * starting_vertex = vertices[starting_nodeId];
		...
		while(!dijkstra_queue.empty()){
			auto tmp = dijkstra_queue.top();
			tmp->set_known();

			for(auto table : tmp->getEdges()){
				auto e_vertex = table.second;

				if(!e_vertex->dijkstra_vertex->is_known()){
					double weight = e_vertex->weight;
					auto vertex = e_vertex->dijkstra_vertex;

					if(weight + tmp->get_path_weight() < vertex->get_path_weight()){
						vertex->set_path_weight(weight + tmp->get_path_weight());
						vertex->set_path_ptr(tmp);
					}

					dijkstra_queue.push(vertex);
				}
			}
			dijkstra_queue.pop();
		}
